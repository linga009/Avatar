...
def run_bootstrap(cfg, n_steps=5000, checkpoint_dir="data/checkpoints/halo3", seed=42):
    ...
    schedule = optax.warmup_cosine_decay_schedule(init_value=0.0, peak_value=cfg.lr, warmup_steps=500, decay_steps=n_steps, end_value=cfg.lr*0.1)
    ...
    for step in range(n_steps):
        ...
        updates, opt_state = opt.update(eqx.filter(grads, eqx.is_array), opt_state, eqx.filter(model, eqx.is_array))
