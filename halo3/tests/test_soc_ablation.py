...
def test_soc_disabled_k_unchanged():
    ...
    # Use values within block-specific bounds: analytical [0.02, 0.20], creative [0.50, 4.00]
    for i in range(10):
    ...
    result = cop.observe(r_mean=0.3, r_a=0.15, r_c=0.15, fe_delta=0.0,
                         K_aa=0.10, K_cc=1.0, K_cross=0.25, theta=_make_theta())
    assert abs(result["K_aa"] - 0.10) < 1e-6, f"K_aa changed: {result['K_aa']}"
    assert abs(result["K_cc"] - 1.0) < 1e-6, f"K_cc changed: {result['K_cc']}"
...
def test_soc_enabled_k_changes():
    ...
    cop = CriticalDynamics(cfg)
    # Start within bounds: analytical [0.02, 0.20], creative [0.50, 4.00]
    for i in range(10):
        cop.observe(r_mean=0.3, r_a=0.15, r_c=0.15, fe_delta=0.0,
                    K_aa=0.05, K_cc=0.80, K_cross=0.25, theta=_make_theta())
    result = cop.observe(r_mean=0.3, r_a=0.15, r_c=0.15, fe_delta=0.0,
                         K_aa=0.05, K_cc=0.80, K_cross=0.25, theta=_make_theta())
    # r_a=0.15 < 0.5, so K_aa should increase; r_c=0.15 < 0.5, so K_cc should increase
    assert result["K_aa"] > 0.05 or result["K_cc"] > 0.80, "K should increase when r < 0.5"
