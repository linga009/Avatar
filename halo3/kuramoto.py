...
def unity_index(C: jnp.ndarray) -> tuple[float, float, float]:
    ...
    PR = (sum |v1|^2)^2 / sum |v1|^4  — how many clusters participate
         in the dominant coherent mode (1 = one cluster, K = all clusters).

    U -> 1 with large gap = unified cognitive state (one subject).
    Multiple comparable eigenvalues = fragmented.
    High PR = broad integration across many clusters.
    Low PR = narrow focus in few clusters.
    ...
    eigenvectors = jnp.flip(eigenvectors, axis=1)  # match eigenvalue order
    total = jnp.sum(eigenvalues)
    U = eigenvalues[0] / (total + 1e-8)
    gap = (eigenvalues[0] - eigenvalues[1]) / (eigenvalues[0] + 1e-8)

    # Participation ratio of leading eigenvector
    v1 = eigenvectors[:, 0]
    v1_sq = v1 * v1  # |v1[k]|^2 (real eigenvectors for symmetric C)
