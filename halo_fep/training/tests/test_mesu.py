...
import optax
import pytest
from halo_fep.training.mesu import mesu, MESUState
...
def test_mesu_update_scales_by_inverse_sigma():
    ...
    opt = mesu(lr=1.0, eta=0.0, epsilon=1e-9)  # no sigma adaptation
    params = {"w": jnp.zeros(1)}

    # Low sigma (uncertain parameter) -> large update
    state_low = MESUState(sigma={"w": jnp.ones(1) * 0.1})
    grads = {"w": jnp.ones(1)}
    updates_low, _ = opt.update(grads, state_low)

    # High sigma (certain parameter) -> small update
    state_high = MESUState(sigma={"w": jnp.ones(1) * 10.0})
